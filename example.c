...
static int check_channel_layout(const AVCodec *codec, const uint64_t channel_layout)
{
    ...
    while (*p != 0)
    { // 0作为退出条件，比如libfdk-aacenc.c的aac_channel_layout
        printf("%s support channel_layout %ld\n", codec->name, *p);
        if (*p == channel_layout)
            return 1;
        p++;
    }
...
{
    const char *in_pcm_file = "test_new.pcm"; // 输入PCM文件
    const char *out_aac_file = "test.aac";    // 输出的AAC文件
    ...
    printf("2 frame_size:%d\n\n", codec_ctx->frame_size); // 决定每次到底送多少个采样点每个通道
    ...
    int8_t *pcm_buf = (int8_t *)malloc(frame_bytes);
    if (!pcm_buf)
    {
        printf("pcm_buf malloc failed\n");
        return 1;
    }
    int8_t *pcm_temp_buf = (int8_t *)malloc(frame_bytes);
    if (!pcm_temp_buf)
    {
